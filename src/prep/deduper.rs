...
use workflow::{RealTaskId, RealTaskKey};

use super::{ActualTaskId, Error};
...
impl Deduper {
    ...
    pub fn get_actual_task_id(&self, id: RealTaskId) -> Result<ActualTaskId, Error> {
        self.id_map.get(id).copied().ok_or(Error::MissingActualTaskId(id))
    }
}
